...
| `ctx.llmOAuth` | `core` | [`llm-oauth`](../packages/llm/llm-oauth) | - | [`llm-pi-ai`](../packages/llm/llm-pi-ai), [`command-login`](../packages/llm/command-login) | - | File-backed pi-ai CredentialStore at $DSH_HOME/.auth.yaml; login/logout persist tokens the pi-ai adapter hands to createModels. |
