...
`@deepseek-ai/dsh-tui` is a new bundle under `packages/bundle/tui/`. Its patch rides directly over base: coding persona, process tool mode, HMR off, Code Mode worker, `tui-startup`, and `tui-runtime`. It mounts no Host, HTTP server, Web runtime, or browser plugin. Model-facing tools join through [agent presets](2026-08-14-tui-agent-presets.md). The runtime reads `ctx.agentDefaultModel`, creates or resumes one persisted Agent through `ctx.agents`, and owns the TTY through `@oh-my-pi/pi-tui` under bun ([later decision](2026-08-14-tui-omp-engine-and-catalog.md)). The interactive layout and dark theme follow [Pi's coding-agent interactive mode](https://github.com/earendil-works/pi): accent header with key hints, user-message `Box`, Markdown assistant blocks, `presentCall`/`presentResult` tool cards with pending/success/error backgrounds, muted-border editor, cwd/model footer. Typed lines become user messages; `/` lines stay in `dsh-commands` (`/help`, `/exit`, `/quit` register here). Ask-user prompts use `dsh-user-questions` overlays. Ctrl+C cancels a running Agent and exits when idle; Ctrl+D exits. Teardown stops pi-tui, drains input, flushes the Session, and requests exit through launcher-owned `ctx.appExit`. A missing TTY is a usage error.
...
**Switch the renderer to `@oh-my-pi/pi-tui`.** Rejected at the time because that package ships TypeScript source, declares a bun engine, and pulls `pi-natives`; the then-shipped Node CLI consumed compiled `@earendil-works/pi-tui`, which already provided Markdown, Editor, `Box`, and display-column wrap. Pi's interactive-mode layout and dark theme (header, Markdown chat, tool cards, muted editor border, cwd/model footer) are the application layer on top of that engine. [The later TUI bun decision](2026-08-14-tui-omp-engine-and-catalog.md) supersedes this rejection for the TUI process only.
...
Package tests cover transcript formatting, Markdown chat blocks, tool-card mapping, header/footer chrome, slash autocomplete, question overlays, the Loader startup provider (`--resume` / `--session` / `--help`), and a FakeTerminal session (create/resume, submit, slash dispatch, Ctrl+C/Ctrl+D, teardown). The built and source `dsh` bins pin a non-TTY bare invocation to `tui requires an interactive TTY` when bun can re-exec, or to the missing-bun diagnostic otherwise; `--profile tui --help` and `--dump-default-config` (contains `dsh-tui`, no Host/Web/client rows) still omit `tui` from launcher-owned subcommands. There is no keyless assembled TUI snapshot example; presentation is the package semantic matrix, and Terminal is the test seam.
