...
- The tracker reports each running run to the footer through `runsChanged(running, summaries)`. `SessionFooter.setSubagentRuns` paints one dim row — `⏵ label: status` entries joined by ` · `, in start order — between the status chips and the model row while any run is live; an empty array hides it. Each summary's `status` is `running <tool>` while a child `tool/call` is pending (no `tool/result` yet) and `thinking` otherwise, derived from the `pendingTools` map the tracker already keeps, so no new event consumption is needed. The window title keeps the count (`dsh` when idle, `dsh · <n> subagent(s) running` while live), as does OSC progress, which is also on while the parent turn is busy. A `subagent/end` that actually settles a live run writes C0 BEL (`\a`); duplicate and unknown ends stay silent. Teardown restores the idle title and clears progress before `tui.stop()`.
...
**Desktop notification or a `/agents` roster.** The shipped reminder is the engine's existing `setTitle` / `setProgress` plus a C0 BEL; the OMP-parity Alt+A roster and full transcript overlay shipped as [TUI Agent Hub](2026-08-15-tui-agent-hub.md).
...
`tests/subagents.spec.ts` (bun) covers card open/settle with footer counts, descriptor labelling, activity folding with tool presentation, failing-result lines, window rolling with an earlier-count, foreign-session rejection, duplicate/unknown lifecycle edges, and the per-run `thinking` → `running <tool>` → `thinking` status transitions — including that `end()` returns whether a run actually settled. `tests/chrome.spec.ts` covers the dedicated subagents row, its hiding on an empty array, and the `subagentsRow` join. An app-level case in `tests/tui.spec.ts` emits the lifecycle pair and child appends through the real `SessionStore` dispatch and asserts the footer per-run row (`⏵ subagent · in-process: thinking`, then `⏵ <label>: running bash`), running/idle window title, progress flag, BEL in the captured write stream, and rendered card text.
