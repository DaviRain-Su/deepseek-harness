...
随附的 [`dsh-tui`](../../../../packages/bundle/tui/README.md) 组合包注入该服务，并把 `/help`、`/exit` 和 `/quit` 注册为命令定义。自动补全与帮助视图读取实时目录，因此插件命令会随其 effect 出现和消失。任何以 `/` 开头的提交行都留在命令平面；未知输入产生终端警告，不会落入模型消息。

TUI 释放会通过 runtime 的中止信号取消未完成的分派，并在请求进程退出之前停止 pi-tui。

## 测试

注册表测试覆盖语法边界、不可变规范化、运行时元数据校验、确定性排序、全局与作用域遮蔽、重复拒绝、准确释放、变更通知失败隔离、直接调用、预期和格式错误结果、同步与异步失败，以及每种中止时序边沿；达到逐文件 100% 语句、分支、函数和行覆盖率。

随附 TUI 包测试覆盖斜杠分派、`/help` 列表、未知命令拒绝、命令错误结果，以及 `/exit` / `/quit`。呈现由 FakeTerminal 上的包内语义矩阵覆盖，而不是无密钥的组装 TUI 快照。
...
- ACP 自动化服务器、无头 CLI（命令行界面）与 JSON-RPC SDK 运行入口不暴露命令平面；随附 TUI 消费它。Web 斜杠输入使用单独的客户端管道。
