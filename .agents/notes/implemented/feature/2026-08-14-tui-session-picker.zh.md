...
`/sessions` 和 `/sessions <id>` 直接调用 `ctx.sessionQuery`。picker 用 `filterSessions` 过滤本进程 cwd 且 `parent: null`（只看顶层），再加上 `isSwitchableSession`，避免出现 subagent `origin`。标题来自 `readTitleSnapshots`；整批失败时仍列出 id。选中当前 id 是空操作。正在运行的一轮会提示 `finish the current turn before switching sessions` 且不切换。
...
**列出全部持久化会话。** 否决：picker 是本工作区的顶层对话，不是 subagent 子会话或其他 cwd。

**自动打开浏览器那种分组工作区列表。** 否决：工作区归 Web；TUI picker 是扁平的 cwd 目录。

## 后果

交互式 `dsh` 可以在不重启的情况下切换对话。启动路径仍是 `--resume`。其他 cwd 的会话仍用 `--resume <id>` 打开。斜杠自动补全还会建议用户可调用的 skill 名称（[TUI skill 斜杠补全](2026-08-14-tui-skill-slash-complete.md)）。

## 测试

`packages/bundle/tui/tests/sessions.spec.ts` 钉住 cwd / parent / origin 过滤，以及 title 与 id 标签。`transcript.spec.ts` 和 `subagents.spec.ts` 钉住 `reset()`。`tui.spec.ts` 在 `pnpm run test:tui` 下启动 FakeTerminal 会话：缺少 `sessionQuery` 会提示、picker 对当前 id 按 Enter 保持 Agent、`/sessions <id>` 在运行中拒绝，随后切换并更新页眉。仍没有无密钥的组装 TUI 快照。
