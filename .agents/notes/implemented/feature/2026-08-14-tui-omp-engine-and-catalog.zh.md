...
TUI **进程**在 bun 上运行。Node 版 `dsh` 启动器在 `parseDshArgs` 选出 profile `tui` 且 `process.versions.bun` 未设置时，用 `bun` `exec` 同一入口和 argv。缺少 bun 或版本低于 1.3.14 则以退出码 1 报错并指出 [https://bun.sh](https://bun.sh)。源码启动（`pnpm dsh`）仍先走 `node --import tsx/esm` 再 re-exec；web、headless、dump-config、plugin 以及启动器 `--help` 留在 Node。
...
Chrome 适配跟随 OMP 的 `getEditorTheme` / `getMarkdownTheme` / `getSelectListTheme` / `getSymbolTheme`：圆角框符号、muted 编辑器边框、`hintStyle`、用户消息 `Box`、Markdown 助手块、工具卡片背景。内置调色板是 Pi 的 `dark`，加上从 OMP 拷贝的 `dark-tokyo-night`、`dark-catppuccin` 和 `light` 色值；`/theme` 切换活的 `TUI_COLOR` 对象。不交付 OMP 的主题加载器、监视器、mermaid ASCII 或 native highlight FFI。pi-tui 把短于 TTY 的帧顶对齐，因此 `SessionChrome` 在 transcript 和编辑器之间插入空行，直到 transcript 填满视口。`reasoning-delta` 分片和组装后的 `reasoning` 块会以暗色斜体 Thinking 正文出现在 Markdown 回答之前。

`dsh-llm-pi-ai` 增加 `enableInstalledCatalog`（默认 `false`）。为 true 时，对每个 `catalogProviderTakesApiKey` 接受的 `catalogProviderIds()` 条目注册一条省略 `apiKeyEnv` 的路由，从而走 pi-ai 的环境发现（`OPENAI_API_KEY` 等）；显式 `providers` 条目覆盖对应占位。已交付组合将该标志保持关闭。`/model`（以及 Ctrl+P / Alt+P）列出已注册的 `ctx.llm.listProviders()` × `listModels()` 路由——DeepSeek 正式路由，外加 settings 里配置的 pi-ai profile——写入 `ModelSelectionRef.current`，调用 `agentDefaultModel.saveSelection()`，并更新页脚。
...
**在 tui patch 里打开已安装目录。** 不予采纳，因为 `/model` 应该提供已配置、真正可用的路由，而不是每一条 pi-ai catalog 模型。额外提供方写进 `$DSH_HOME/settings.yaml` 的 `llm-pi-ai:`，与 web Models 页的写法相同。

**在 base 里打开已安装目录。** 不予采纳，因为 web Models 页把空的 `providers` 字典当作休眠，并把 `$DSH_HOME/settings.yaml` 当作活路由集。扩大该默认会改变 web 组合。
...
TUI `/model` 只列出 `ctx.llm` 当前已注册的路由。要加 OpenAI/Anthropic/Gemini/… 是 settings profile，不是 TUI 把目录整表铺出来。

Node coverage 不测量 `packages/bundle/tui/src`。该树的正确性由 bun 下的 `pnpm run test:tui` 负责。

## 测试

`apps/cli` 单测 bun 版本下限；source-launch / built-bin 冒烟在 bun 已 re-exec 时接受 `tui requires an interactive TTY`，否则接受缺 bun 诊断。`dsh-llm-pi-ai` 单测休眠默认、目录展开，以及设置层覆盖占位。bun 下的 TUI 包测试覆盖调色板 `/theme`、overlay picker、对着假 `ctx.llm` 的 `/model`、FakeTerminal 会话、chrome、把编辑器钉在短视口的最下面几行，以及流式和回放的 reasoning。仍然没有无密钥的组装 TUI 快照；呈现仍是包内语义矩阵。
