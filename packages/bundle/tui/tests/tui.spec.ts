...
describe('tui runtime', () => {
  ...
  it('notices the settings file path from the hub', async () => {
    const test = await bench()
    const { app, code } = await test.run()
    test.ctx.provide('settings', { documentPath: '/tmp/dsh/settings.yaml' } as never)
    await app.submit('/settings')
    test.fake.type('\x1b[B')
    test.fake.type('\x1b[B')
    test.fake.type('\x1b[B')
    ...
    test.fake.type('\r')
    expect(app['overlay']).toBeUndefined()
    expect(app['transcript'].container.render(80).join('\n')).toContain('settings file /tmp/dsh/settings.yaml')
    await app.quit(0)
    expect(await code).toBe(0)
    await test.ctx.fiber.dispose()
  })
