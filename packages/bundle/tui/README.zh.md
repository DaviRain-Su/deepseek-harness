...
- **Models 设置只写密钥和 base URL，不编辑完整 profile**：`/settings` → Models 经 `ctx.credentials` 存 API key，经 `ctx.settings.mutate` 存 `baseURL`；不改模型列表、协议，也不手改 `settings.yaml`。Sections 列出已注册命名空间和字段名，不打开按 schema 驱动的编辑器，也不显示字段值。Settings file 只提示本地路径，不打开编辑器。订阅 OAuth 仍走 `/login`。
...
- **context 占用可从活模型预热**：`ctx` 在有 `request/context` 记录时用 `contextPressure.contextWindow`，否则用 `llm.resolveModelInfo` 的容量，因此首轮计费 sample 之前也可以出现该分组。
