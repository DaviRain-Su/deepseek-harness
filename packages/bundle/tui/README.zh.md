...
- **Models 设置只是名册**：`/settings` → Models 列出 `ctx.llm.listConfigurableProviders()`；改凭证仍走 `/login` 和 `settings.yaml`。
