...
- **Models 设置只写密钥和 base URL，不编辑完整 profile**：`/settings` → Models 经 `ctx.credentials` 存 API key，经 `ctx.settings.mutate` 存 `baseURL`；不改模型列表、协议，也不手改 `settings.yaml`。Settings file 只提示本地路径，不打开编辑器。订阅 OAuth 仍走 `/login`。
