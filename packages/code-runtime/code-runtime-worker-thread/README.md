...
- **Type-strip host-side, in execution context** — the program is wrapped in an async-function shell, stripped with `node:module`'s `stripTypeScriptTypes` when that export exists, otherwise `amaro` strip-only (erasable syntax only — `enum`/namespaces are rejected as a program `exception` and no worker spawns), and sliced back out byte-positioned; it then executes as the body of an `AsyncFunction`, so top-level `await`/`return` work.
...
- **Type-strip rides Node's experimental `stripTypeScriptTypes` API when present** — bun does not export it; that path uses `amaro` strip-only.
- **bun leaves `worker.stdout` / `worker.stderr` null** — even with `stdout: true`, so the host skips pipe capture there. JS-level `console` and patched stream writes still reach the message port; native writes that bypass those slots are not captured under bun.
