...
The dict shape makes duplicate routes unrepresentable, and the pre-release array shape (with per-profile `provider` fields) fails load with migration directions. `providers` may also be empty or omitted entirely: the written section then contributes no routes. Plugin apply still registers every installed catalog provider that takes an API key and already has that key in the process environment (the product CLI materializes project and user `.env` there); explicit `providers` entries override those stubs, and emptying the settings section drops only the settings-supplied routes. The catalog route `deepseek` is omitted from that ambient set because the official adapter already serves `DEEPSEEK_API_KEY` as `deepseek-official`. A set `OLLAMA_API_KEY` also registers the declared route `ollama-cloud` (hosted Ollama Cloud; not a pi-ai catalog id); an explicit `providers.ollama-cloud` entry wins. `enableInstalledCatalog` (default `false`) instead registers every installed catalog provider that takes an API key as a route with no `apiKeyEnv`, so pi-ai ambient discovery (`OPENAI_API_KEY` and siblings) applies; explicit `providers` entries override the matching stub, and a request on an unkeyed route still fails with `MISSING_CREDENTIAL`. Shipped compositions leave the flag off; a deployment that wants every installed catalog route sets it explicitly. Dormant or not, the plugin declares every installed catalog provider in the configurable-provider directory (`ctx.llm.listConfigurableProviders()`, settings path `providers.<provider>`), joined with every route the current profiles declare, so configuration surfaces can offer the full catalog before any route exists and can still address a hand-declared one. Each entry carries `declared`: whether pi-ai ships nothing under that key. It follows the installed catalog, never the settings document, because narrowing a shipped provider's models stores a profile too and that route is still one pi-ai knows — only the adapter can tell the two apart, which is why the directory answers rather than leaving a surface to infer it. Which adapters exist is composition; which providers run can be entirely the user's settings document. Registration with `ctx.llm` is atomic: a collision with any provider route already owned by another adapter fails plugin loading without registering the remaining routes. Model ids are not lifecycle config; a model the route does not configure fails before any provider request with `LlmError('UNKNOWN_MODEL')`, except on `ollama-cloud`, where `listModels` is advisory and an unlisted id still resolves.

## Catalog resolution

The `xai` route also offers `grok-4.6` (Responses, with `xhigh`) until the installed `@earendil-works/pi-ai` catalog ships that id; an installed entry of the same id wins.
...
- **A provider that authenticates through OAuth alone is withheld until a credential is stored** — the configurable-provider directory still skips catalog routes that declare only OAuth (`openai-codex` is the only one the installed catalog ships). `dsh login` persists a token in `$DSH_HOME/.auth.yaml`; when `@deepseek-ai/dsh-llm-oauth` is mounted (the shipped base does, before this adapter), apply lists those ids, registers an empty catalog stub, and hands the store to `createModels({ credentials })` so pi-ai refreshes under the store lock. Web still cannot start the flow. A route a settings document already names keeps its directory entry, and `apiKeyEnv` still authenticates it with that key — which for Codex is a token that expires with nothing on the key path to refresh it.
...
- **A route's catalog never refreshes itself** — the catalog is whatever `settings.yaml` says, so a model list is only as current as its last edit. Nothing here queries a provider for the models it serves, except the ambient `ollama-cloud` route, whose `listModels` reads `GET https://ollama.com/v1/models`. A settings-declared route gains a model when someone writes one.
...
- **pi-ai ships no Ollama catalog** — hosted Ollama Cloud still auto-registers as `ollama-cloud` when `OLLAMA_API_KEY` is set (`https://ollama.com/v1`). A local Ollama daemon is a hand-declared OpenAI-compatible route (`baseURL` the host's `/v1`, models listed in the profile) in the `llm-pi-ai:` settings section; `OLLAMA_HOST` alone does not register one.
