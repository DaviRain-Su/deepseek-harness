...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/bundle/tui': { kind: 'none', reason: 'The terminal runtime submits typed lines as ordinary user messages; prompts and tools belong to the composed base and tui bundles.' },
  ...
  'packages/llm/llm-oauth': { kind: 'indirect', reason: 'The store persists OAuth tokens; the pi-ai adapter owns any model-facing request auth those tokens authorize.' },
  'packages/llm/command-login': { kind: 'indirect', reason: 'The CLI commands persist OAuth tokens; the pi-ai adapter owns any model-facing request auth those tokens authorize.' },
